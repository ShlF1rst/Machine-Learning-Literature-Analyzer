...
def prep_features_scikit(features):
	...
	result = v.fit_transform(features)
	return result
...
def create_connections_file(directory):
	with open(directory+'/names.txt') as f:
		names = set(f.read().split('\n'))
	names = set([x.strip('\n\r') for x in names])
	...
	while trigger:
		...
		with open(directory+'/names.txt','w') as f:
			for name in names:
				f.write(name+'\n')
...
class LitAnalysisApp(QtWidgets.QMainWindow, main_design.Ui_MainWindow):
	...
	def processText(self):
		...
		with open('profiles\\'+self.bookName.text()+'\\book.txt','w') as f:
			f.write('1 TMP\n\n' + text)
		if test_connect():
